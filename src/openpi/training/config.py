...
# ────────────────────────────────────────────────────────────
# OpenArm π₀ configs  (added by setup_openpi.sh)
# ────────────────────────────────────────────────────────────
...
class LeRobotOpenarmDataConfig(DataConfigFactory):
    ...
    use_wrench=True 로 wrench 토큰 추가 (Pi0Config(wrench_dim=12) 와 함께 사용).
    """
    repo_id: str = "local:/home/kimminju/openarm_pi0_dataset"
    action_dim: int = 16
    use_delta_actions: bool = True
    cameras: tuple[str, ...] = openarm_policy.DEFAULT_CAMERAS
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_map = {
            **{f"observation/{c}": c for c in self.cameras},
            "observation/state": "state",
            "actions":           "actions",
            "prompt":            "prompt",
        }
        if self.use_wrench:
            repack_map["observation/wrench"] = "wrench"
        ...
        if self.use_delta_actions:
            mask = _transforms.make_bool_mask(7, -1, 7, -1)
            data_tf = data_tf.push(inputs=[_transforms.DeltaActions(mask)], outputs=[_transforms.AbsoluteActions(mask)])
        return dataclasses.replace(
            self.create_base_config(assets_dirs, model_config),
...
_CONFIGS = [
    TrainConfig(
        ...
        data=LeRobotOpenarmDataConfig(
            ...
            assets=AssetsConfig(asset_id="openarm_pi0_dataset"),
    ...
    TrainConfig(
        ...
        data=LeRobotOpenarmDataConfig(
            repo_id=_WHITEBOARD_DATASET,
            cameras=("image", "right_wrist_image", "left_wrist_image", "center_image"),
    ...
    TrainConfig(
        ...
        data=LeRobotOpenarmDataConfig(
            repo_id="local:/home/kimminju/data/0529_left_grasp_lerobot",
            action_dim=8,
    ...
    # ── 0601 멀티태스크: grasp + wipe_blue + wipe_red (왼팔, f_ext_L) ──────────
    TrainConfig(
        name="0601_multitask",
        model=pi0_config.Pi0Config(
            action_dim=8, action_horizon=50,
    ...
    # ── 0615 force test: 왼팔, f_ext_L, 1-cam (left wrist only) ──────────────
    TrainConfig(
        name="0615_force_test",
        model=pi0_config.Pi0Config(
            action_dim=8, action_horizon=50,
            wrench_dim=6,
            paligemma_variant="gemma_2b_lora",
        ...
        data=LeRobotOpenarmDataConfig(
            ...
            action_dim=8,
            use_delta_actions=False,
            cameras=("left_wrist_image",),
            use_wrench=True,
            assets=AssetsConfig(asset_id="0615_force_test_lerobot"),
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ...
    # ── 0615 force test (no wrench): 같은 데이터셋, wrench 토큰 없이 ───────────
    TrainConfig(
        name="0615_force_test_nowrench",
        model=pi0_config.Pi0Config(
            action_dim=8, action_horizon=50,
            wrench_dim=0,
            paligemma_variant="gemma_2b_lora",
